...
export default function CategoriesPage() {
  ...
  const handleAddCategoryKeyDown = useCallback(
    (event: React.KeyboardEvent) => {
      if (event.key === "Enter") void handleAddCategorySubmit();
      if (event.key === "Escape") {
        setIsAddingCategory(false);
        setNewCategoryName("");
      }
  ...
  const handleAddCategoryBlur = useCallback(() => {
  ...
  const handleAddTaskKeyDown = useCallback(
    (event: React.KeyboardEvent) => {
      if (event.key === "Enter") void handleAddTaskSubmit();
      if (event.key === "Escape") {
        setIsAddingTask(false);
        setNewTaskTitle("");
      }
  ...
  const handleAddTaskBlur = useCallback(() => {
